...
class SLR:
    ...
    def parse(self, buffer: List[Symbol]) -> List[str]:
        ...
        while i < len(buffer):
            ...
            if action.state == ParsingTableActionState.ERROR:
            ...
            elif action.state == ParsingTableActionState.GOTO or \
                    action.state == ParsingTableActionState.SHIFT:
                result.append(f"{action.state}: from {from_index} to {action.index} with {buffer_symbol}.")
                ...
                result.append(f"END: from {from_index} to ACCEPTED {buffer_symbol}")
